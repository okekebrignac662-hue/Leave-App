...
## 🛡️ ระบบป้องกันความผิดพลาดระดับโปรดักชัน (Production Hardening)

ระบบได้รับการออกแบบและป้องกันข้อผิดพลาดที่มักพบบนเซิร์ฟเวอร์ออนไลน์อย่างครบถ้วน:

1. **ป้องกันเซิร์ฟเวอร์หลับ (Cold Start Defense):**
   - มี Endpoint ความเร็วสูง `/api/ping` และ `/ping`
   - หน้าเว็บมีระบบ **Pre-Warm Detection** อัตโนมัติ แจ้งเตือนผู้ใช้ด้วยแถบข้อความนุ่มนวลหากเซิร์ฟเวอร์กำลังตื่น
   - ดูวิธีตั้งค่า UptimeRobot ฟรีได้ที่ 👉 [`KEEP_ALIVE_GUIDE.md`](KEEP_ALIVE_GUIDE.md)
2. **ป้องกันการแย่งโควตา (Race Condition & Concurrency Shield):**
   - ใช้ PostgreSQL Row-Level Lock (`FOR UPDATE`) ใน Transaction (`BEGIN ... COMMIT`) จัดคิวคำขอในแผนกเดียวกันอย่างเคร่งครัด โควตาไม่มีทางล้นเกินกำหนดแม้กดส่งพร้อมกันในเสี้ยววินาที
3. **ป้องกันเวลาเพี้ยน (Asia/Bangkok Timezone Enforced):**
   - บังคับ Timezone ประเทศไทย (UTC+7) ทั้งระดับ Node.js, Session ฐานข้อมูล และ Date Parser ป้องกันวันที่ร่นถอยหลัง
4. **ป้องกันเน็ตหลุดหน้างานและการกดรัว (Network Timeout & Double-Click Guard):**
   - ตัดการทำงานด้วย `AbortController` ภายใน 35 วินาที พร้อมแจ้งเตือนภาษาไทยที่เข้าใจง่าย
   - ล็อคปุ่มทันทีที่คลิก เปลี่ยนเป็น *"⏳ กำลังประมวลผล... ห้ามกดย้ำ"* ป้องกันการส่งเบิ้ล
   - ตรวจจับสัญญาณ Wi-Fi/4G หลุดแบบเรียลไทม์ พร้อมตรวจสอบคำขอซ้ำบนเซิร์ฟเวอร์
